...
public class Time implements ITime {
    ...
    public int difference(ITime time) {
        Time t = (Time) time;
        return (int)Math.sqrt(Math.pow((this.gc.getTimeInMillis() - t.gc.getTimeInMillis()) / 60000, 2));
    }
    ...
    {
        Time other2 = (Time) other;

        if(this.getYear() != other2.getYear())
        {
            return false;
        }
        
        if(this.getMonth() != other2.getMonth())
        {
            return false;
        }
        
        if(this.getDay() != other2.getDay())
        {
            return false;
        }

        if(this.getHours() != other2.getHours())
        {
            return false;
        }
        
        if(this.getMinutes() != other2.getMinutes())
        {
            return false;
        }
